...
public class SklstfServiceImpl implements SklstfService {
	...
	// 관리자가 직원 등록
	@Override
	public void insertSklstf(SklstfVO sklstfVO, Sklstf_AtrtyVO sklstf_AtrtyVO)
			throws SQLException {
	...
	// 관리자가 직원 정보와 권한 수정
	@Override
	public void updateSklstf_Atrty(SklstfVO sklstfVO,
			Sklstf_AtrtyVO sklstf_AtrtyVO) throws SQLException {
		sklstfDAO.updateSklstf(sklstfVO);
	...
	// 전체 직원의 목록 조회
	@Override
	public List<UserSubjctVO> listAllSklstf(int tpage, SearchVO searchVO)
			throws SQLException {
	...
	public String pageNumber(int tpage, SearchVO searchVO) throws SQLException {
		int totalRecord = sklstfDAO.totalSklstf(searchVO);
		String page = new Paging().pageNumber(tpage, totalRecord, "sklstfList",
				"&key=" + searchVO.getKey() + "&value=" + searchVO.getValue());
		return page;
	}

	// 직원 권한 목록 조회 페이지 설정
	@Override
	public String pageNumberAtrty(int tpage, SearchVO searchVO)
			throws SQLException {
		int totalRecord = sklstfDAO.totalSklstf(searchVO);
		String page = new Paging().pageNumber(tpage, totalRecord,
				"sklstfAtrtyList", "&key=" + searchVO.getKey() + "&value="
						+ searchVO.getValue());
		return page;
	}
	...
	public void returnSklstf(String stf_Sklstf_No) throws SQLException {
		sklstfDAO.returnSklstf(stf_Sklstf_No);
	}
	
	// 탈퇴직원목록 조회 페이지 설정
	@Override
	public String pageNumber2(int tpage, SearchVO searchVO) throws SQLException {
		int totalRecord = sklstfDAO.totalSklstf(searchVO);
		String page = new Paging().pageNumber(tpage, totalRecord, "deleteSklstfList",
				"&key=" + searchVO.getKey() + "&value=" + searchVO.getValue());
		return page;
	}
}
