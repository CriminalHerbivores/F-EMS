...
function add_IntrstLctre(){
	...
	$(".result_1:checked").each(function(){
		...
		var temp_1=$(tr).find('.lc_Lctre_No');
		var in_Lctre_No= $(temp_1).text();
	...
	var data_1 = JSON.stringify(array_1);
	  ...
	  $.ajax({
	        ...
	        success:function(res){
	            ...
	            $.each(res, function(index,intrst){
	            	tr_row_2 += '<tr id="intrstList_tr"><td class="select_ckbox_1">'
	...
	var row_2 = new Array();
		...
		// 수강신청 체크박스
		$(".result_2:checked").each(function(){
			var tr = $(this).closest("tr");
			var re_Stdnt_No= "${loginUser}";
			var temp_2=$(tr).find('.lc_Lctre_No');
			var re_Lctre_No= $(temp_2).text();
			
			row_2= { "re_Stdnt_No" : re_Stdnt_No, "re_Lctre_No" : re_Lctre_No };
			array_2.push(row_2);
			});	// result_2:checked
			console.log("array_2 : " + array_2);

		var data_2 = JSON.stringify(array_2);
	    ...
	    $.ajax({
	        ...
	        success:function(res){
				...
				$.each(res, function(index,lctre){
					// 개설강의 목록
		        	var tr_row_1="";
		        	$.each(res, function(index,openLctre){
		        		
		        		tr_row_1 += '<tr class="slt_ckbox_${status.index}"><td class="select_ckbox_1" id="lc_${status.index}">'	
									+'<label><input type="checkbox" class="input_check_1 result_1" name="result_1" value="'+openLctre.lc_Lctre_No+'"/>관심</label></td>'
									+'<td class="select_ckbox_2" id="re_${status.index}"><label>'
									+'<input type="checkbox" class="input_check_2 result_2" id="ck_all_${status.index}" name="result_2" value="'+openLctre.lc_Lctre_No+'" />수강</label></td>'
									+'<td class="lc_Lctre_No">'+openLctre.lc_Lctre_No+'</td>'
									+'<td>'+openLctre.sit_Subjct+'</td>'
									+'<td>'+openLctre.lu_Lctre_Code+'-'+openLctre.lc_Split+'</td>'
									+'<td>'+openLctre.lu_Lctre_Nm+'</td>'
		        	...
		        	tr_row_1 += '<tr><td colspan="13" style="text-align: center;">${paging}</td></tr>';
		        	
		        	$("#openList").empty();
		            $("#openList").html(tr_row_1);
		            ...
		            $.each(res, function(index,lctre){
		            	pnt = lctre.lu_Pnt.split("/")[0];
		            	sumOfReqst += pnt*1;
		            });
		            	row_4 +='<tr><td><h3>18 - '+sumOfReqst+' = '+(18-sumOfReqst)+'</h3></td></tr>';
		            
		            $("#ableOfReqst").empty();
		            $("#ableOfReqst").html(row_4);
		            
		            // 수강 신청 완료 학점
		            var row_5="";
		            //$.each(res, function(index,lctre){
		            	row_5 += '<tr><td><h3>'+sumOfReqst+'</h3></td></tr></tbody>';
		            //});
	
		            $("#sumOfReqst").empty();
		            $("#sumOfReqst").html(row_5);
	...
	var data = JSON.stringify(array);
	  ...
	  $.ajax({ 
			...
			$.each(res, function(index,lctre){
	            ...
	            // 수강완료 
	            var row_3="";
				row_3 += '<tr class="slt_ck_${status.index}"><td class="select_ck_2">'
	            		+'<label><input type="checkbox" class="input_ck_2 result_3" id="all_ck_${status.index}"  name="result_3" value="'+lctre.in_Lctre_No+'" />삭제</label></td>'
	            		+'<td class="select_ck_1"><label><input type="checkbox" class="input_ck_1 result_4" name="result_4" value="'+lctre.re_Lctre_No+'" />취소</label></td>'
	            		+'<td class="lc_Lctre_No">'+lctre.lc_Lctre_No+'</td>'
	            		+'<td>'+lctre.sit_Subjct+'</td>'
...
function del_reqstLctre(){
	...
	$(".result_4:checked").each(function(){
		var tr = $(this).closest("tr");
		var re_Stdnt_No= "${loginUser}";
	...
	var data = JSON.stringify(array);
	  ...
	  var array_4 = [];
		...
		// 한건의 tr 태그가 하나의 javascript 객체 형성
		// 여러건의 tr 태그, 즉 javascript 객체들이 하나의 배열 형성
			row_4= { "in_Stdnt_No" : in_Stdnt_No, "in_Lctre_No" : in_Lctre_No };
			array.push(row_4);
			});	// result_1:checked
			console.log(array_4);

		// 만들어진 배열을 json 형태로 변환
		var data_4 = JSON.stringify(array);
...
<div class="text-center" >
			...
			<thead>
				<tr>
					<th colspan="2" class="text-left"><input type="button" class="def-btn ckbtn-color" value="선택 삭제" onclick="del_reqstLctre()"></th>
...
<div class="text-center"><table class="def-table-full tb-border">
	<thead><tr><th>신청 가능 학점
	</th></tr></thead>
	<tbody id="ableOfReqst"><tr><td><h3>18 - ${sumOfReqst} = ${ableOfReqst}</h3>
	</td></tr></tbody>
</table>
<br/>
<table class="def-table-full tb-border">
	<thead><tr><th>수강 신청 학점
	</th></tr></thead>
	<tbody id="sumOfReqst"><tr><td><h3>${sumOfReqst}</h3>
	</td></tr></tbody>
</table>
